  ...
  return (
    <>
      <div className="absolute right-0 bottom-full flex flex-row gap-10 mb-20">
        {teachers.map((teacherOption, idx) => (
          <div
            key={idx}
            className={`p-3 transition-colors duration-500 ${
              teacher === teacherOption ? "bg-white/80" : "bg-white/40"
            }`}
          >
            <div onClick={() => setTeacher(teacherOption)}>
              <img
                src={`/images/Fem.jpg`}
                alt={teacherOption}
            ...
            <h2 className="text-3xl font-bold mt-3 text-center">{teacherOption}</h2>
          </div>
        ))}
      </div>
