...
export function Teacher({ teacher, ...props }) {
  const group = useRef();
  const { scene } = useGLTF(`/models/Teacher_Fem.glb`);
  useEffect(() => {
  ...
  const { animations } = useGLTF(`/models/animations_Fem.glb`);
  ...
  return (
    <group {...props} dispose={null} ref={group}>
      {loading && (
        <Html position-y={teacher === "Avatar" ? 1.6 : 1.8}>
...
teachers.forEach((teacher) => {
  useGLTF.preload(`/models/Teacher_Fem.glb`);
  useGLTF.preload(`/models/animations_Fem.glb`);
});